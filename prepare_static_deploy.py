#!/usr/bin/env python3
"""
Prepare static deployment package for Vercel/GitHub Pages.
This creates a self-contained static site with all data embedded.
"""
...
def prepare_deployment():
    ...
    print("📦 Preparing static deployment...")
    ...
    static_files = [
        'static/index.html',
        'static/styles.css',
        'static/amsterdam_pc4.geojson',
        'static/district_recommendations.json'
    ]
    
    for file in static_files:
        if os.path.exists(file):
            dest = os.path.join(deploy_dir, os.path.basename(file))
            shutil.copy2(file, dest)
            print(f"    ✓ {file}")
    
    # Copy restaurant data
    print("  Copying restaurant data...")
    if os.path.exists('restaurants_data.json'):
        shutil.copy2('restaurants_data.json', os.path.join(deploy_dir, 'restaurants_data.json'))
        print("    ✓ restaurants_data.json")
    ...
    # Update index.html to use local data files
    print("  Updating paths for static deployment...")
    index_path = os.path.join(deploy_dir, 'index.html')
    with open(index_path, 'r') as f:
        html = f.read()
    
    # Update script src to be relative
    ...
    # Create a fresh app.js specifically for static deployment
    print("  Creating static app.js...")
    static_app_js = """
...
"""
    ...
    with open(os.path.join(deploy_dir, 'app.js'), 'w') as f:
        f.write(static_app_js)
    
    # Create README for deployment
    readme = """# Amsterdam Restaurants Map

Live interactive map of Amsterdam restaurants with AI-powered market insights.

## Features
- 🗺️ Interactive map with 3,600+ restaurants
...
"""
    ...
    print(f"\n📁 Files created in: {os.path.abspath(deploy_dir)}/")
    print("\n🚀 Next steps:")
    print("   1. cd deploy")
    print("   2. vercel --prod")
    print("\n   Or push to GitHub and deploy via Vercel dashboard")
    print("\n💡 Tip: Install Vercel CLI with: npm install -g vercel")
